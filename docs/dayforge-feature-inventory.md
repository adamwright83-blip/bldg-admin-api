...
- **PR E:** persisted tenant proposal profiles; immutable mission-derived proposal snapshots; administrator-only version generation and approval; tenant-scoped idempotency and audit events; approved-only driver access; draft watermarking; honest browser Print/Save PDF fallback; and a server-enforced requirement that current approved collateral exist before the Field checklist can claim it is ready. No hard-coded operator profile or localStorage handoff remains authoritative.
