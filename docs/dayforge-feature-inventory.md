...
| ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `.github/workflows/dayforge-foundation.yml`                                     | Focused DayForge typecheck, tests, build, and diagnostic artifacts                              | Reusable experimental IP; superseded by extraction-specific CI                                     | PR A-G CI as applicable                          |
...
After these checks, PR #10 may be closed as **superseded by the archive plus PRs A-G**, never as discarded work.
...
- **PR G:** one controlled mission-to-revenue pipeline; stable account, location, and contact identity; tenant-scoped relationship history and follow-ups; won-account conversion into customer, service expectation, planned route, agreement, and first-order records; exactly-once final mission reward; and an explicit value ladder separating estimated opportunity, evidenced approved agreement, unavailable invoice truth, paid orders, and realized revenue.
- **PR H:** third-party DayForge tenant provisioning; persisted multi-location capacity, routes, services, branding, and proposal configuration; owner/admin/operator/field memberships; standalone tenant login and exact-once invitations; provider-neutral customer/order imports with a tenant-safe CleanCloud CSV adapter; namespaced Stripe Checkout, Billing Portal, replay-safe subscription webhooks, ordered subscription truth, delinquency policy, and server-enforced entitlements. The locked landing-page implementation remains unchanged.
