...
- `LegacyDayforgeLoginPage.tsx` currently redirects every successful login to `/julydemo`.
...
| Command | Result |
| --- | --- |
| `npm run check:legacy-dayforge:release` | Pass |
| `npm run test:legacy-dayforge:release` | Pass: 23 files, 97 tests |
...
| `npm run legacy-dayforge:migrations:verify` | Environment has `DATABASE_URL`, but the configured database is unavailable |
...
- `shared/commercialMission.ts`, `shared/commercialPipeline.ts`, `shared/commercialProposal.ts`, `shared/legacyLegacyDayforgeCoaching.ts`, and `shared/legacyLegacyDayforgeContinuation.ts` — shared contracts and validation.
- `server/commercialMissions/*`, `server/commercialCampaigns/*`, and `server/legacyLegacyDayforgeCoaching/*` — canonical persistence and service foundations.
- `server/legacyLegacyDayforgeRetention/retentionService.ts`, `server/storage.ts`, and `server/_core/sms.ts` — private proof lifecycle and dispatch adapters.
...
- `pnpm db:legacy-dayforge:release` — all 46 migrations, including `0045`, applied cleanly to a fresh database.
- `pnpm test:legacy-dayforge:release:integration` — passes.
