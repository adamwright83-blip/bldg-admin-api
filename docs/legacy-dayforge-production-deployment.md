...
git switch -c legacy-dayforge-cumulative-integration origin/claude/dayforge-boss-demo-completion
...
# 4. Re-verify:
DATABASE_URL="..." pnpm legacy-dayforge:migrations:verify
...
railway run --service bldg-admin-api pnpm legacy-dayforge:demo:setup
railway run --service bldg-admin-api pnpm legacy-dayforge:demo:verify
```
...
`playwright.legacy-dayforge.config.ts` already supports targeting a deployed URL —
...
DAYFORGE_RELEASE_BASE_URL=https://api.bldg.chat \
  ...
  pnpm test:legacy-dayforge:release:e2e
```
