...
4. **Conversation Relay.** Leave it off. `CLAIRE_TWILIO_CONVERSATION_RELAY` stays unset. Gather remains the production default. The upgrade handler is mounted while the flag is off. Leave `CLAIRE_TWILIO_RELAY_PUBLIC_BASE_URL` unset until, after this handler is deployed, the anonymous probe receives the Goldline 403 marker `goldline-relay-upgrade-rejected`. That 403 is transport reachability. `101` is a later authenticated check. See `docs/claire/conversation-relay-reachability.md`.
