...
### Scenery (accepted — 2026-09-09, crop/repack pass filling the gap left when Codex's session ran out of credits)

- `client/public/assets/goldline/chapters/the-last-valet/mechanisms/scenery-atlas.png`: 1536×1024, same 3×2/512×512-cell convention as the other atlases here. Cell 1 raised machinery-island platform, cell 2 foreground occlusion rail, cell 3 main gate (closed), cell 4 shortcut crate, cell 5 decorative wheel; cell 0 unused/reserved.
- Source: `client/public/assets/goldline/chapters/the-last-valet/references/mediterranean_art_deco_rooftop_asset_sheet.png` — a ChatGPT-generated **source sheet** (not a pre-made atlas; it has no knowledge of this renderer's cell geometry), provided by Adam on 2026-09-09. Generation id: `b1776ffd-7b5d-4478-8cdd-2b9ec6dbc5bf`. Exact prompt used to generate it: unknown (generated externally, outside this session).
- Genuinely transparent RGBA (verified: corner pixels alpha=0, object-interior pixels alpha≈253), unlike the mechanism/props/fx atlases above which used opaque white for multiply-compositing — no compositing trick was needed here.
- Extraction method: `scripts/goldline/extract-scenery-atlas.py`, a deterministic, re-runnable crop/repack script. It crops five tight bounding boxes from the source sheet, applies a 3px alpha erosion (`ImageFilter.MinFilter(3)`) to remove the yellow/green matting-halo fringe visible around every object in the source sheet, trims to each object's visible bounding box, and composites each into its target 512×512 cell (scaled to fit with padding, anchored toward the cell's bottom edge to match the other atlases' anchor convention). Re-run the script (adjusting the `CROPS` dict) if any cell needs recropping.
- Disposition of other objects on the same source sheet (bridge/platform, potted palm, cypress-in-planter, wall lamp on pillar, bollard, small flower planter, second fence variant, separate lever prop): not extracted — no current `ChapterScene.ts` slot calls for them. Left in the preserved source sheet for a future pass if a use appears; do not re-generate the source sheet to get these, they already exist there.
- Code change: `ChapterScene.ts`'s `scenery` asset path changed from the never-generated placeholder `props/scenery-atlas.png` to this real file; the shortcut-crate sprite call moved from a reused frame 1 (previously shared with the island, back when neither existed and both used the same vector fallback) to its own frame 4, since distinct source art now exists for each. Vector fallbacks for all five remain in the code, firing only if this atlas fails to load.
