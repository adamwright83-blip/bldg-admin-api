...
async function assertWorldFills(page: Page, size: { width: number; height: number }) {
...
test.describe("manifest-driven corridor runtime", () => {
  test("boots corridor_01 from its manifest and renders exactly one canvas", async ({
    ...
    const canvasCount = await page.locator("canvas.goldline-game-canvas").count();
  ...
  test("requests corridor_01's manifest — the world is addressed by id, not by asset URL", async ({
    ...
    page.on("request", request => {
      if (request.url().includes("/assets/goldline/") && request.url().endsWith("manifest.json")) {
    ...
    expect(manifestRequests.some(url => url.includes("corridor_01/manifest.json"))).toBe(true);
  });
  ...
  test("does NOT preload every corridor at boot", async ({ page }) => {
    ...
    page.on("request", request => {
      ...
      if (url.includes("/assets/goldline/corridor_")) corridorRequests.push(url);
    });

    await login(page);
    await page.waitForTimeout(2000);

    // corridor_02 exists in the registry but is authoring-stage; nothing from
    // it may be fetched, and no corridor beyond the active one may be warmed.
    expect(corridorRequests.some(url => url.includes("corridor_02"))).toBe(false);
...
test.describe("mobile viewport survives world activity", () => {
  ...
  test("the Pixi renderer resizes with the viewport rather than keeping a stale backing size", async ({
    ...
    const portraitBox = await page.locator("canvas.goldline-game-canvas").boundingBox();

    await page.setViewportSize(LANDSCAPE);
    await page.waitForTimeout(600);

    const landscapeBox = await page.locator("canvas.goldline-game-canvas").boundingBox();
...
test.describe("Pixi lifecycle stays clean across repeated mounts", () => {
  test("five navigations away and back leave exactly one renderer", async ({ page }) => {
    await login(page);
  ...
  test("repeated mounts do not accumulate uncaught page errors", async ({ page }) => {
    const errors: string[] = [];
