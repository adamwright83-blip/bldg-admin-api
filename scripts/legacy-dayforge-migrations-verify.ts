...
async function main() {
  ...
  if (!db) {
    console.error("[legacy-dayforge-migrations-verify] Database not available");
    process.exit(1);
  }
  ...
  console.log(
    ok
      ? "[legacy-dayforge-migrations-verify] OK"
      : "[legacy-dayforge-migrations-verify] FAILED"
  );
  process.exit(ok ? 0 : 1);
}

main().catch(error => {
  console.error("[legacy-dayforge-migrations-verify] Failed:", error);
  process.exit(1);
});
