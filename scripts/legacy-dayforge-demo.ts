...
function fail(message: string): never {
  console.error(`[legacy-dayforge-demo] ${message}`);
  process.exit(1);
}
...
async function main() {
  console.log("[legacy-dayforge-demo] 1/7 Validating environment...");
  ...
  console.log("[legacy-dayforge-demo] 2/7 Connecting to database...");
  ...
  try {
    ...
    fail("Migrations verify failed. Run `pnpm db:legacy-dayforge:release` and retry.");
  }

  console.log("[legacy-dayforge-demo] 4/7 Resetting and re-seeding the demo tenant...");
  const preExisting = await verifyDemoTenant();
  if (preExisting.checks.some(c => c.name === "demo_tenant_exists" && c.pass)) {
    await resetDemoTenant({ role: "admin", id: "cli:legacy-dayforge-demo" });
  }
  const seedResult = await seedDemoTenant();

  console.log("[legacy-dayforge-demo] 5/7 Verifying demo tenant integrity...");
  ...
  console.log("[legacy-dayforge-demo] 6/7 Starting the application server...");
  ...
  if (!alreadyUp) {
    ...
    console.log(`[legacy-dayforge-demo]     Server already running at ${BASE_URL}, reusing it.`);
  }

  console.log("[legacy-dayforge-demo] 7/7 Smoke check...");
...
main().catch(error => {
  console.error("[legacy-dayforge-demo] Failed:", error);
  process.exit(1);
});
