    ...
    <main className="min-h-screen bg-[#07111f] text-slate-100 pb-28">
      <header className="sticky top-0 z-20 border-b border-white/10 bg-[#07111f]/95 px-4 py-4 backdrop-blur">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3">
          <div><p className="text-xs font-black tracking-[.2em] text-orange-400">{tenant.data?.configuration?.tenant.brandName ?? PRODUCT_NAME}</p><h1 className="text-2xl font-black">What creates revenue next?</h1></div>
            ...
            <div className="space-y-3">{items.map(item => (
              <article key={item.id} className="rounded-2xl border border-white/10 bg-slate-900 p-4 shadow-xl">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  ...
                  {item.pipelineId && item.followUpId ? <button type="button" onClick={async () => { await completeFollowUp.mutateAsync({ pipelineId: item.pipelineId!, followUpId: item.followUpId!, requestId: crypto.randomUUID(), outcome: "contacted_no_decision", notes: "Completed from Today queue: contacted, no decision." }); await queue.refetch(); }} className="rounded-xl border border-emerald-400/40 px-3 py-3 text-sm font-bold text-emerald-200">Contacted, no decision</button> : null}
