...
export class PlayerController {
  ...
  update(move: MoveVector, cameraYaw: number, dt: number) {
    ...
    if (mag > 0) {
      ...
      targetSpeed = WALK_SPEED * mag * clamp(1.15 - misalign / 1.9, 0.15, 1);
    } else {
  ...
  private moveHorizontal(dx: number, dz: number) {
    ...
    const g = this.groundAt(target.x, target.z, start.y + MAX_STEP_UP + 0.3);
