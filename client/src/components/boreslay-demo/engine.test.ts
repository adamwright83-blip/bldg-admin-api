...
describe("PublicBoreslayEngine", () => {
  ...
  it("EXCUSE collision damages once during invulnerability", () => { const e=playing();const s=e.state.spark;e.state.projectiles.push({id:1,kind:"excuse",x:s.x,y:s.y,vx:0,vy:0,radius:32},{id:2,kind:"excuse",x:s.x,y:s.y,vx:0,vy:0,radius:32});e.update(16);expect(e.state.spark.hp).toBe(84); });
  ...
  it("defeat triggers at zero Spark health", () => { const e=playing();e.state.spark.hp=16;const s=e.state.spark;e.state.projectiles.push({id:1,kind:"excuse",x:s.x,y:s.y,vx:0,vy:0,radius:32});e.update(16);expect(e.state.status).toBe("defeat"); });
