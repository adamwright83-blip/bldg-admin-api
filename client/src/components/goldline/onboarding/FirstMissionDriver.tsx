...
export function FirstMissionDriver({session}:{session:GoldlineOnboardingSession}){
 ...
 const displayTitle="The first spark";
 ...
 <GoldlineOverworld playerIdentity={`${session.tenantId}:${session.id}`} suppressCampaignChrome onEnterGreystar={()=>{}} onResolveOrder={async()=>false} dayObjectiveCount={1}
  activeObjective={{id:mission.id,kind:"growth",title:displayTitle,sourceLabel:mission.outcome?"FIELD EVIDENCE RECORDED":"OPTIONAL SIDE QUEST",dueAt:null,status:mission.outcome?"completed":"ready",address:mission.checkpoint.label,latitude:mission.checkpoint.latitude,longitude:mission.checkpoint.longitude,physicalEntityId:null,explanation:mission.objective,sourceEvidenceReference:"Find one useful opportunity. Record what you actually observe.",sourceOccurredAt:session.completedAt}}
  ...
  onOpenDayBriefing={()=>{if(mission.gameplayCompletedAt)window.location.href="https://admin.bldg.chat/growth/lantern-city";else if(mission.outcome)setEncounter(true);else setBriefing(true);}} />
 {briefing&&<section className="gl-field-sheet" role="dialog" aria-modal="true" aria-label="First mission field outcome"><button className="gl-sheet-close" onClick={()=>setBriefing(false)}>BACK TO THE GAME</button><p className="gl-eyebrow">TERRITORY SCOUT</p><h1>{displayTitle}</h1><p>{mission.objective}</p><p className="gl-story-echo">The resistance: “{mission.avoidance}”</p>
 ...
 <p>Movement in the game does not count as a real visit.</p><form onSubmit={e=>{e.preventDefault();report.mutate({missionId:mission.id,text,confirmedPresence:true,gps});}}>
 ...
 {mission.outcome&&!encounter&&<aside className="gl-first-payoff"><strong>{mission.gameplayCompletedAt?`${guardian.name} defeated` : "Evidence secured. The Guardian is vulnerable."}</strong><p>{mission.outcome.text}</p>{!mission.gameplayCompletedAt&&<button onClick={()=>setEncounter(true)}>CONFRONT {guardian.name.toUpperCase()}</button>}<a href="https://admin.bldg.chat/growth/lantern-city">RETURN TO LANTERN CITY →</a></aside>}
