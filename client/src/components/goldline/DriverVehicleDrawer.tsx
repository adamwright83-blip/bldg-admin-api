...
export function DriverVehicleDrawer({
  ...
  const [claireState, setClaireState] = useState<"ready" | "calling" | "called">("ready");
  ...
  async function handleClaireBrief() {
    ...
    try {
      ...
      setClaireState("called");
      toast.success("Claire is calling you now.");
    } catch (error) {
      setClaireState("ready");
      toast.error(
        error instanceof Error ? error.message : "Claire could not place the call."
      );
    }
  }
  ...
  return (
    <>
      <div className="gdp-world-tools" aria-label="Goldline world tools">
        <button
          type="button"
          className={`gdp-world-tool gdp-claire-tool is-${claireState}`}
          onClick={() => void handleClaireBrief()}
          disabled={callClaire.isPending}
          aria-label="Call Claire for today's briefing"
        >
          <img src={claireHologram} alt="" aria-hidden="true" />
          <span className="gdp-world-tool-state">
            {claireState === "calling"
              ? "CALLING…"
              : claireState === "called"
                ? "CALL AGAIN"
        ...
        <button
          type="button"
          className="gdp-world-tool gdp-manifest-tool"
          onClick={() =>
            clickHiddenDayControl(
              ".gdp-import",
              "Today's route import is unavailable."
            )
          }
          aria-label="Open today's manifest and import route work"
        >
          <img src={todayManifest} alt="" aria-hidden="true" />
          <span className="gdp-manifest-live-copy">
            <b>IMPORT / SYNC</b>
        ...
        <button
          type="button"
          className="gdp-world-tool gdp-overland-tool"
          onClick={() =>
            clickHiddenDayControl(
              ".gdp-world-entry > button",
              "Overland is unavailable right now."
            )
          }
          aria-label="Enter Overland"
        >
          <img src={overlandPortal} alt="Overland" />
        </button>

        <button
          type="button"
          className="gdp-world-tool gdp-first-spark-tool"
          onClick={() => window.dispatchEvent(new Event("goldline:first-spark"))}
          aria-label="Open The First Spark side quest"
        >
