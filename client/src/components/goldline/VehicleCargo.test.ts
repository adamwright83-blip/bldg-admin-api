...
describe("Vehicle Cargo home projection", () => {
  it("keeps an empty vehicle empty", () =>
    expect(visibleCargo([])).toEqual({ visible: [], overflow: 0 }));
  it("uses deterministic paper bag art for unprocessed custody", () => {
    expect(cargoSprite(item(1, "IN_VEHICLE_UNPROCESSED"))).toContain(
      "paper-bag-a"
    );
    expect(cargoSprite(item(2, "IN_VEHICLE_UNPROCESSED"))).toContain(
      "paper-bag-b"
    );
  });
  it("uses finished art only for processed custody", () =>
    expect(cargoSprite(item(3, "IN_VEHICLE_PROCESSED"))).toContain(
      "processed-hanging"
    ));
  it("uses the same cargo sprite system for unlinked field cargo", () =>
    expect(
      cargoSprite({ ...item(3, "IN_VEHICLE_UNPROCESSED"), id: "field:yazi" })
    ).toContain("cargo-unprocessed"));
  it("exposes truthful overflow after four readable slots", () => {
    const result = visibleCargo(
      [1, 2, 3, 4, 5, 6].map(id => item(id, "IN_VEHICLE_UNPROCESSED"))
    );
