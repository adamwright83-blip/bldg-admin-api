...
export const saasRouter = router({
  providerStatus: dayforgeTenantMemberProcedure.query(() => ({
    ...
    billing: {
      provider: "stripe",
      status: process.env.DAYFORGE_BILLING_STRIPE_SECRET_KEY
        ? ("configured" as const)
        : ("not_configured" as const),
      mode: process.env.DAYFORGE_BILLING_STRIPE_SECRET_KEY?.startsWith(
        "sk_live_"
      )
        ? ("live" as const)
        : process.env.DAYFORGE_BILLING_STRIPE_SECRET_KEY
          ? ("test" as const)
          : ("none" as const),
    },
    sms: {
    ...
    email: {
      provider: "agentmail",
      status:
        process.env.AGENTMAIL_API_KEY || process.env.AGENTMAIL_VENDOR_INBOX_ID
          ? ("configured" as const)
          : ("not_configured" as const),
    },
    printFulfillment: {
      provider: "manual",
      status: "manual_fulfillment" as const,
      connected: false,
    },
    imports: {
      provider: "cleancloud_csv",
      status: "file_import_available" as const,
      connected: false,
    },
  })),
