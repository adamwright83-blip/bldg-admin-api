...
async function failClosedOnMissingTable<T>(work: () => Promise<T>, fallback: T): Promise<T> {
  try {
    return await work();
  } catch (error) {
...
export function createDrizzleClaireRelationshipStore(): ClaireRelationshipStore {
  return {
    async appendEvent(input) {
      ...
      if (!db) return null;
      return failClosedOnMissingTable(async () => {
    ...
    async upsertState(state) {
      const db = await getDb();
      if (!db) return;
      await failClosedOnMissingTable(async () => {
        const values = {
