...
export async function answerClairePreDriveFollowUp(
  ...
  function buildFollowUpSystemPrompt(): string {
    return [
      ...
      "TEMPORAL AUTHORITY: currentContext.clock is the sole authority for the current date, local time, weekday, daypart, and whether a commitment is today, tomorrow, future, or past. Ignore any ambient model/provider/server notion of the current time, even if it seems more recent. Never override currentContext.clock with outside time awareness.",
