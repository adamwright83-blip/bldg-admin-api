...
export type RetentionCountInput = {
  resource: LegacyDayforgeRetentionResource;
  cutoff: Date;
  now: Date;
  limit: number;
};

export interface LegacyDayforgeRetentionStore {
...
type LegacyDayforgeDb = NonNullable<Awaited<ReturnType<typeof getDb>>>;
...
export class MysqlDayforgeRetentionStore implements LegacyDayforgeRetentionStore {
  ...
  constructor(
    private readonly db: LegacyDayforgeDb,
    private readonly deleteEvidenceObject: EvidenceObjectDeleter = storageDelete
  ) {}

  private tableFor(resource: LegacyDayforgeRetentionResource): string {
    switch (resource) {
...
export type LegacyDayforgeRetentionRun = {
...
export async function runDayforgeRetention(input: {
  ...
  const resources: LegacyDayforgeRetentionRun["resources"] = [];
