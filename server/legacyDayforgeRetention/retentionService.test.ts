...
class FakeStore implements LegacyDayforgeRetentionStore {
  counts = new Map<LegacyDayforgeRetentionResource, number>();
  ...
  it("fails closed when the cleanup secret is absent or wrong", () => {
    ...
    expect(
      authorizeRetentionRequest(
        { "x-legacy-dayforge-retention-secret": "correct" },
        "correct"
      )
    ).toBe("authorized");
  });
