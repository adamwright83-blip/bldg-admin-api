...
export type PrintState = "BROWSER_PDF_FALLBACK" | "CONNECTED" | "NOT_CONFIGURED";
...
/** server/territory/googlePlacesTerritoryProvider.ts reads GOOGLE_MAPS_API_KEY. */
export function googleProviderStatus(env: NodeJS.ProcessEnv = process.env): ProviderState {
  return present(env.GOOGLE_MAPS_API_KEY) ? "LIVE" : "NOT_CONFIGURED";
}
 ...
 */
export function stripeProviderStatus(env: NodeJS.ProcessEnv = process.env): ProviderState {
...
export function emailProviderStatus(env: NodeJS.ProcessEnv = process.env): ProviderState {
  const required = [
    "AGENTMAIL_API_KEY",
...
export function smsProviderStatus(env: NodeJS.ProcessEnv = process.env): ProviderState {
