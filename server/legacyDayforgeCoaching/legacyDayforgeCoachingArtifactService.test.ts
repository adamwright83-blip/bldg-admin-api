...
function reference(
  id: string,
  sourceType: LegacyDayforgeCoachingGroundingEvidence["reference"]["sourceType"] = "general_industry_guidance",
): LegacyDayforgeCoachingGroundingEvidence["reference"] {
  return {
    id,
    sourceType,
...
function directEvidence(): LegacyDayforgeCoachingGroundingEvidence[] {
  return [
    {
...
function modelOutput(extraClaims: Array<{
  key: LegacyDayforgeCoachingGroundingEvidence["claimKey"];
  displayValue: string;
...
describe("DayForge coaching grounding policy", () => {
  ...
  it("builds stable, honest fallback content without account PII", () => {
    ...
    const stableContent = (output: LegacyDayforgeCoachingOutput) => ({
