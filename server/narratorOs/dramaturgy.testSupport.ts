/**
 * Test-only dramaturgy rule injection.
 *
 * This is the only module that exports a function accepting a caller
 * rule catalog. That function throws outside test/Vitest. Production
 * modules do not export it. Do not import this file from production
 * Narrator paths. `server/narratorOs/index.ts` does not re-export it.
 */
import type {
  DramaturgyDecision,
...
export function decideDramaturgyWithRulesForTests(input: {
  ...
  return renderWithRules(input.eligibility, input.tieBreaks);
}
