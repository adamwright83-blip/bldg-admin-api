...
export const missionDirectorRouter = router({
  planForDate: legacyLegacyDayforgeTenantMemberProcedure
  ...
  revisions: legacyLegacyDayforgeTenantMemberProcedure
    .input(z.object({ businessDate: date }))
    .query(({ ctx, input }) =>
      listPlanRevisions({
        tenantId: ctx.tenantId,
  ...
  recordUsage: legacyLegacyDayforgeTenantMemberProcedure
    .input(
      z.object({
        businessDate: date,
